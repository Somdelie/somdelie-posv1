...
export function middleware(request: NextRequest) {
  ...
  if (jwt) {
    try {
      ...
      const currentRole = user.authorities || user.role;
      if (currentRole === "ROLE_USER" && (user.storeId || user.store)) {
        user.role = "ROLE_STORE_ADMIN";
        user.authorities = "ROLE_STORE_ADMIN";
      }
      ...
      console.log("Middleware - Has permission:", hasAccess);
      console.log(
        "Middleware - Allowed routes for role:",
        roleRoutePermissions[userRole]
      );

      if (!hasAccess) {
        console.log("Middleware - Access denied, redirecting to not-found");
